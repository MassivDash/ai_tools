...
test('loads suites on open', async () => {
  const suites = [
    { id: '1', name: 'Suite 1', description: 'Test Suite 1', created_at: Date.now() },
    { id: '2', name: 'Suite 2', description: 'Test Suite 2', created_at: Date.now() }
  ]
...
test('creates a new suite', async () => {
  ...
  mockedAxios.get.mockResolvedValueOnce({ 
      data: [{ id: '1', name: 'New Suite', description: 'Desc', created_at: Date.now() }] 
  })
...
test('loads questions when clicking a suite', async () => {
  ...
  const questions = [
      { id: '101', suite_id: '1', content: 'What is the capital of France?', created_at: Date.now() }
  ]
  
  mockedAxios.get.mockImplementation((url: string) => {
      if (url === 'agent/testing/suites') return Promise.resolve({ data: suites })
      if (url === 'agent/testing/suites/1/questions') return Promise.resolve({ data: questions })
...
test('runs 4 questions without skipping', async () => {
   ...
   const runBtn = screen.getByText('Run Suite')
   await fireEvent.click(runBtn)
   expect(runQuestionSpy).toHaveBeenLastCalledWith(expect.objectContaining({ detail: { content: 'Q1' } }))
   
   // Next -> Q2
   ;(component as any).handleRunnerNext()
   expect(runQuestionSpy).toHaveBeenLastCalledWith(expect.objectContaining({ detail: { content: 'Q2' } }))
   
   // Next -> Q3 (This is the reported skip point)
   ;(component as any).handleRunnerNext()
   expect(runQuestionSpy).toHaveBeenLastCalledWith(expect.objectContaining({ detail: { content: 'Q3' } }))
   
   // Next -> Q4
   ;(component as any).handleRunnerNext()
   expect(runQuestionSpy).toHaveBeenLastCalledWith(expect.objectContaining({ detail: { content: 'Q4' } }))
 })
