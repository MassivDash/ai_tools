...
test('renders all tool cards when no tool is selected', () => {
  ...
  expect(screen.getByText('Upload PDF files and convert to markdown')).toBeTruthy()
})
...
test('displays placeholder for HTML to Markdown', async () => {
  ...
  await waitFor(() => {
    expect(screen.getByText('🚧 HTML to Markdown tool coming soon')).toBeTruthy()
    expect(screen.getByText(/This tool will allow you to paste HTML content/)).toBeTruthy()
  })
})
...
test('displays placeholder for PDF to Markdown', async () => {
  ...
  await waitFor(() => {
    ...
    expect(screen.getByText(/This tool will allow you to upload PDF files/)).toBeTruthy()
  })
})
